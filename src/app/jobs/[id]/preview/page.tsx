...
export default async function JobApplicationPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error("Job application preview failed to load job:", error);
  }
  ...
  return (
    <main className="min-h-screen bg-sand px-5 py-10 text-navy sm:px-8">
      <section className="mx-auto max-w-2xl rounded-3xl border border-ink/10 bg-paper p-8 shadow-soft">
        <p className="text-xs font-black uppercase text-ink">Preview unavailable</p>
        <h1 className="mt-3 text-3xl font-black sm:text-4xl">Could not load job preview</h1>
